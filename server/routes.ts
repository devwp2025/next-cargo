...
export async function registerRoutes(
  ...
  app.post("/api/dashboard/products", requireAuth, async (req, res) => {
    try {
      ...
      const product = await storage.createProduct({ ...data, sellerId: req.session.userId! });
  ...
  app.patch("/api/dashboard/products/:id", requireAuth, async (req, res) => {
    try {
      ...
      const product = await storage.updateProduct(parseInt(req.params.id), req.session.userId!, data);
      if (!product) return res.status(404).json({ message: "Product not found" });
      res.json(product);
    } catch (err: any) {
      res.status(400).json({ message: err.message || "Invalid input" });
  ...
  app.patch("/api/dashboard/products/:id/status", requireAuth, async (req, res) => {
    const { status } = z.object({ status: z.enum(["active", "hidden", "sold"]) }).parse(req.body);
    const product = await storage.updateProductStatus(parseInt(req.params.id), status, req.session.userId!);
    if (!product) return res.status(404).json({ message: "Product not found" });
    res.json(product);
  });
  ...
  app.patch("/api/dashboard/orders/:id/status", requireAuth, async (req, res) => {
    try {
      const { status } = z.object({ status: z.enum(["preparing", "shipped", "completed", "canceled"]) }).parse(req.body);
      ...
      log(`Order ${order.id} status updated to ${status} by seller ${req.session.userId}`);
      res.json(updated);
    } catch (err: any) {
      res.status(400).json({ message: err.message || "Invalid input" });
    }
  ...
  app.post("/api/orders", requireAuth, async (req, res) => {
    try {
      const { productId } = z.object({ productId: z.number().int() }).parse(req.body);
      const product = await storage.getProductById(productId);
      if (!product) return res.status(404).json({ message: "Product not found" });
      if (product.sellerId === req.session.userId) return res.status(400).json({ message: "Cannot buy your own product" });

      const reserved = await storage.reserveProduct(productId);
      if (!reserved) return res.status(400).json({ message: "Product not available" });

      const order = await storage.createOrder(productId, req.session.userId!, product.sellerId, product.price);
  ...
  app.post("/api/pay/mock/:sessionId/action", requireAuth, async (req, res) => {
    try {
      const { action } = z.object({ action: z.enum(["succeed", "fail", "cancel"]) }).parse(req.body);
  ...
  app.post("/api/chat/start", requireAuth, async (req, res) => {
    try {
      ...
      const product = await storage.getProductById(productId);
      if (!product) return res.status(404).json({ message: "Product not found" });
      if (product.sellerId === req.session.userId) return res.status(400).json({ message: "Cannot chat with yourself" });

      const conv = await storage.getOrCreateConversation(productId, req.session.userId!, product.sellerId);
      res.json(conv);
    } catch (err: any) {
      res.status(400).json({ message: err.message || "Invalid input" });
    }
  ...
  app.get("/api/chat/:conversationId/info", requireAuth, async (req, res) => {
    const conv = await storage.getConversationInfo(parseInt(req.params.conversationId), req.session.userId!);
    if (!conv) return res.status(404).json({ message: "Conversation not found" });
    if (conv.buyerId !== req.session.userId && conv.sellerId !== req.session.userId) {
  ...
  app.get("/api/chat/:conversationId/messages", requireAuth, async (req, res) => {
    const conv = await storage.getConversationById(parseInt(req.params.conversationId));
    if (!conv) return res.status(404).json({ message: "Conversation not found" });
    if (conv.buyerId !== req.session.userId && conv.sellerId !== req.session.userId) {
      return res.status(403).json({ message: "Access denied" });
    }
    const after = req.query.after as string | undefined;
    const msgs = await storage.getMessages(conv.id, after && after !== "" ? after : undefined);
    res.json(msgs);
  });
  ...
  app.post("/api/chat/:conversationId/messages", requireAuth, async (req, res) => {
    try {
      ...
      const recentCount = await storage.getRecentMessageCount(req.session.userId!, 1);
      if (recentCount >= 30) {
        return res.status(429).json({ message: "Too many messages. Please wait." });
      }

      const sanitized = escapeHtml(text);
      const msg = await storage.createMessage(conv.id, req.session.userId!, sanitized);
  ...
  app.patch("/api/admin/products/:id/status", requireAdmin, async (req, res) => {
    const { status } = z.object({ status: z.enum(["active", "hidden", "sold"]) }).parse(req.body);
    const product = await storage.updateProductStatus(parseInt(req.params.id), status);
    if (!product) return res.status(404).json({ message: "Product not found" });
    res.json(product);
  });
  ...
  app.post("/api/admin/categories", requireAdmin, async (req, res) => {
    try {
      const { name, slug } = z.object({ name: z.string().min(1), slug: z.string().min(1) }).parse(req.body);
